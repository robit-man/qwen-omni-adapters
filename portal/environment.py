"""Bounded, non-secret runtime facts injected into each portal turn."""
...
def runtime_environment_system_message() -> dict[str, str]:
    serialized = json.dumps(
        runtime_environment_snapshot(), ensure_ascii=True, separators=(",", ":")
    )[:MAX_CONTEXT_CHARS]
    return {
        "role": "system",
        "content": (
            "This trusted runtime snapshot was freshly generated by the local portal "
            "for this turn. Use it for environment-aware answers when relevant, but "
            "do not claim it describes the user's device and do not volunteer noisy "
            "telemetry when it is unrelated.\n<runtime_environment>"
            f"{serialized}</runtime_environment>"
        ),
    }
